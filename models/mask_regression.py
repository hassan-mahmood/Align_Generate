...
class MaskRegressNet(nn.Module):
    ...
    def forward(self, obj_feat, bbox):
        ...
        """
        b, num_o, _ = bbox.size()
        obj_feat = obj_feat.reshape(b * num_o, -1)
        x = self.fc(obj_feat)
        x = self.conv1(x.reshape(b * num_o, 128, 4, 4))
        x = F.interpolate(x, size=8, mode='bilinear')
        x = self.conv2(x)
        x = F.interpolate(x, size=16, mode='bilinear')
        x = self.conv3(x)
        x = x.reshape(b, num_o, 16, 16)

        bbmap = masks_to_layout(bbox, x, self.map_size).reshape(b, num_o, self.map_size, self.map_size)
        return bbmap
...
class MaskRegressNetv2(nn.Module):
    ...
    def forward(self, obj_feat, bbox):
        ...
        obj_feat = obj_feat.reshape(b * num_o, -1)
        x = self.fc(obj_feat)
        x = self.conv1(x.reshape(b * num_o, 256, 4, 4))
        x = F.interpolate(x, size=8, mode='bilinear')
        x = self.conv2(x)
        x = F.interpolate(x, size=16, mode='bilinear')
        x = self.conv3(x)
