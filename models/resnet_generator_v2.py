...
class ResnetGenerator128(nn.Module):
    ...
    def forward(self, z, bbox, z_im=None, y=None):
        ...
        latent_vector = torch.cat((z, label_embedding), dim=1).reshape(b, o, -1)

        w = self.mapping(latent_vector.reshape(b * o, -1))
        # preprocess bbox
        ...
        # 4x4
        x = self.fc(z_im).reshape(b, -1, 4, 4)
        # 8x8
        x, stage_mask = self.res1(x, w, bmask)

        # 16x16
        hh, ww = x.size(2), x.size(3)
        seman_bbox = batched_index_select(stage_mask, dim=1, index=y.reshape(b, o, 1, 1))  # size (b, num_o, h, w)
        seman_bbox = torch.sigmoid(seman_bbox) * F.interpolate(bbox_mask_, size=(hh, ww), mode='nearest')
        alpha1 = torch.gather(self.sigmoid(self.alpha1).expand(b, -1, -1), dim=1, index=y.reshape(b, o, 1)).unsqueeze(-1)
        ...
        seman_bbox = batched_index_select(stage_mask, dim=1, index=y.reshape(b, o, 1, 1))  # size (b, num_o, h, w)
        seman_bbox = torch.sigmoid(seman_bbox) * F.interpolate(bbox_mask_, size=(hh, ww), mode='nearest')
        alpha2 = torch.gather(self.sigmoid(self.alpha2).expand(b, -1, -1), dim=1, index=y.reshape(b, o, 1)).unsqueeze(-1)
        ...
        seman_bbox = batched_index_select(stage_mask, dim=1, index=y.reshape(b, o, 1, 1))  # size (b, num_o, h, w)
        seman_bbox = torch.sigmoid(seman_bbox) * F.interpolate(bbox_mask_, size=(hh, ww), mode='nearest')
        alpha3 = torch.gather(self.sigmoid(self.alpha3).expand(b, -1, -1), dim=1, index=y.reshape(b, o, 1)).unsqueeze(-1)
        ...
        seman_bbox = batched_index_select(stage_mask, dim=1, index=y.reshape(b, o, 1, 1))  # size (b, num_o, h, w)
        seman_bbox = torch.sigmoid(seman_bbox) * F.interpolate(bbox_mask_, size=(hh, ww), mode='nearest')
        alpha4 = torch.gather(self.sigmoid(self.alpha4).expand(b, -1, -1), dim=1, index=y.reshape(b, o, 1)).unsqueeze(-1)
    ...
    def init_parameter(self):
        ...
        for _,k in enumerate(tqdm(self.named_parameters())):
            if k[1].dim() > 1:
...
class ResnetGenerator256(nn.Module):
    ...
    def forward(self, z, bbox, z_im=None, y=None, include_mask_loss=False):
        ...
        alpha5 = torch.gather(self.sigmoid(self.alpha5).expand(b, -1, -1), dim=1, index=y.reshape(b, o, 1)).unsqueeze(-1)
...
def bbox_mask(x, bbox, H, W):
    ...
    bbox_1 = bbox.float().reshape(-1, 4)
    x0, y0 = bbox_1[:, 0], bbox_1[:, 1]
    ww, hh = bbox_1[:, 2], bbox_1[:, 3]

    x0 = x0.contiguous().reshape(N, 1).expand(N, H)
    ww = ww.contiguous().reshape(N, 1).expand(N, H)
    y0 = y0.contiguous().reshape(N, 1).expand(N, W)
    hh = hh.contiguous().reshape(N, 1).expand(N, W)

    X = torch.linspace(0, 1, steps=W).reshape(1, W).expand(N, W).cuda(device=x.device)
    Y = torch.linspace(0, 1, steps=H).reshape(1, H).expand(N, H).cuda(device=x.device)

    X = (X - x0.to(X.device)) / ww.to(X.device)
    Y = (Y - y0.to(Y.device)) / hh.to(Y.device)

    X_out_mask = ((X < 0) + (X > 1)).reshape(N, 1, W).expand(N, H, W)
    Y_out_mask = ((Y < 0) + (Y > 1)).reshape(N, H, 1).expand(N, H, W)

    out_mask = 1 - (X_out_mask + Y_out_mask).float().clamp(max=1)
    return out_mask.reshape(b, o, H, W)
