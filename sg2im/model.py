...
class Layout2ImModel(nn.Module):
  def __init__(self, vocab, image_size=(64, 64), embedding_dim=64,
               ...
               mask_size=16, mlp_normalization='none', layout_noise_dim=0,
               **kwargs):
    super(Layout2ImModel, self).__init__()
    ...
    #   self.mask_net = self._build_mask_net(num_objs, 256, mask_size)


    refinement_kwargs = {
      #'dims': (gconv_dim + layout_noise_dim,) + refinement_dims,
      'dims': (128,) + refinement_dims,
  ...
  def forward(self, obj_vecs,boxes_pred, triples, obj_to_img=None):
    ...
    layout = boxes_to_layout(obj_vecs.cuda(), layout_boxes.cuda(), obj_to_img.cuda(), H, W)
