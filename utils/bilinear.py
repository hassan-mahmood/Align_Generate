...
def crop_bbox(feats, bbox, HH, WW=None, backend='cudnn'):
    ...
    X = tensor_linspace(x0, x1, steps=WW).reshape(N, 1, WW).expand(N, HH, WW).cuda(device=feats.device)
    Y = tensor_linspace(y0, y1, steps=HH).reshape(N, HH, 1).expand(N, HH, WW).cuda(device=feats.device)
...
def tensor_linspace(start, end, steps=10):
    ...
    reshape_size = start.size() + (1,)
    w_size = (1,) * start.dim() + (steps,)
    out_size = start.size() + (steps,)

    start_w = torch.linspace(1, 0, steps=steps).to(start)
    start_w = start_w.reshape(w_size).expand(out_size)
    end_w = torch.linspace(0, 1, steps=steps).to(start)
    end_w = end_w.reshape(w_size).expand(out_size)

    start = start.contiguous().reshape(reshape_size).expand(out_size)
    end = end.contiguous().reshape(reshape_size).expand(out_size)
...
def bilinear_sample(feats, X, Y):
    ...
    y0x0_idx = (W * y0 + x0).reshape(N, 1, HH * WW).expand(N, C, HH * WW)
    y1x0_idx = (W * y1 + x0).reshape(N, 1, HH * WW).expand(N, C, HH * WW)
    y0x1_idx = (W * y0 + x1).reshape(N, 1, HH * WW).expand(N, C, HH * WW)
    y1x1_idx = (W * y1 + x1).reshape(N, 1, HH * WW).expand(N, C, HH * WW)

    # Actually use gather to pull out the values from feats corresponding
    # to our four samples, then reshape them to (BB, C, HH, WW)
    feats_flat = feats.reshape(N, C, H * W)
    v1 = feats_flat.gather(2, y0x0_idx.long()).reshape(N, C, HH, WW)
    v2 = feats_flat.gather(2, y1x0_idx.long()).reshape(N, C, HH, WW)
    v3 = feats_flat.gather(2, y0x1_idx.long()).reshape(N, C, HH, WW)
    v4 = feats_flat.gather(2, y1x1_idx.long()).reshape(N, C, HH, WW)

    # Compute the weights for the four samples
    w1 = ((x1 - X) * (y1 - Y)).reshape(N, 1, HH, WW).expand(N, C, HH, WW)
    w2 = ((x1 - X) * (Y - y0)).reshape(N, 1, HH, WW).expand(N, C, HH, WW)
    w3 = ((X - x0) * (y1 - Y)).reshape(N, 1, HH, WW).expand(N, C, HH, WW)
    w4 = ((X - x0) * (Y - y0)).reshape(N, 1, HH, WW).expand(N, C, HH, WW)
...
def masks_to_layout(boxes, masks, H, W=None):
    ...
    grid = _boxes_to_grid(boxes.reshape(b*num_o, -1), H, W).float().cuda(device=masks.device)

    img_in = masks.float().reshape(b*num_o, 1, M, M)
    sampled = F.grid_sample(img_in, grid, mode='bilinear')

    return sampled.reshape(b, num_o, H, W)
...
def _boxes_to_grid(boxes, H, W):
    ...
    boxes = boxes.reshape(O, 4, 1, 1)

    # All these are (O, 1, 1)
    x0, y0 = boxes[:, 0], boxes[:, 1]
    ww, hh = boxes[:, 2], boxes[:, 3]

    X = torch.linspace(0, 1, steps=W).reshape(1, 1, W).to(boxes)
    Y = torch.linspace(0, 1, steps=H).reshape(1, H, 1).to(boxes)
...
def compute_transformation_matrix(bbox):
    ...
    transformation_matrix = torch.cat([scale_x.unsqueeze(-1), zeros, t_x.unsqueeze(-1),
                                       zeros, scale_y.unsqueeze(-1), t_y.unsqueeze(-1)], 1).reshape(-1, 2, 3)
